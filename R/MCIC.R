MCIC <-
function(inputFile=NA,align=NA,saveFile=TRUE,outname=paste(inputFile,"IndelDistanceMatrixMullerMod.txt"),silent=FALSE)
{
#require(ape)
...
# To include the possibility of no indels in the alignment	
	if(sum(as.numeric(RECOD[1,]))==ncol(RECOD) & sum(as.numeric(RECOD[2,]))==ncol(RECOD))
		{
		DISTANCE[i,j]<-0
		DISTANCE[j,i]<-0
		}
